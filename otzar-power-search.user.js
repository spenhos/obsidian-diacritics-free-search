// ==UserScript==
// @name         Otzar Power Search & Discovery
// @namespace    elevalma.otzar.tools
// @version      0.2.0
// @description  Capa de descubrimiento sobre Otzar HaChochma: cosecha TODOS los resultados de una búsqueda, los mapea por época/siglo, resalta fuentes inesperadas, sugiere búsquedas relacionadas, cruza dos ideas (A∩B) y exporta a CSV/Excel. Reutiliza tu sesión suscrita.
// @author       Saleh Penhos (con Claude)
// @match        https://tablet.otzar.org/*
// @run-at       document-idle
// @grant        none
// ==/UserScript==

/*
 * Recetas técnicas (verificadas en vivo):
 *  - Lista completa de resultados:  pinia._s.get('freeSearchBookList-main').getCurrentListIds  -> [{book,results,start,end}]
 *  - Metadatos del libro:           $store.getters['books/getBookbyId'](id) -> {name, authors_0_name, authors_0_period, places, fromyear, toyear}
 *  - Época (dor):                    dorot[authors_0_period - 1]   (period es 1-indexado)
 *  - Disparar búsqueda:             componente con doFreeSearch(text, [], 0, false, null, 0)
 *  - Saber si terminó:              $store.state.freeSearch.searching === false
 * No descarga libros ni texto: solo organiza referencias de tu propia sesión. Uso personal.
 */
